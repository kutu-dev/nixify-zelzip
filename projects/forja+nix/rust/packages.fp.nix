{flake-parts-lib, ...}: {
  options.perSystem = flake-parts-lib.mkPerSystemOption ({
    ...
    craneLibs = config.forja.rust.crane.libs;

    workspacePath = config.forja.rootPath;

    commonArgs = {
      src = craneLibs.native.cleanCargoSource workspacePath;
      strictDeps = true;
    };
    ...
    cargoArtifacts = {
      native = makeCargoArtifacts craneLibs.native {};

      linux.x86_64 = makeCargoArtifacts craneLibs.linux.x86_64 {};
      linux.aarch64 = makeCargoArtifacts craneLibs.linux.aarch64 {};

      macos.x86_64 = makeCargoArtifacts craneLibs.macos.x86_64 macosX86_64ExtraArgs;
      macos.aarch64 = makeCargoArtifacts craneLibs.macos.aarch64 macosAarch64ExtraArgs;

      windows.x86_64 = makeCargoArtifacts craneLibs.windows.x86_64 {};

      wasm32 = makeCargoArtifacts craneLibs.wasm32 {
    ...
    makeCrateArgs = cargoPackageName: includeProjects:
      commonArgs
      // {
        inherit (craneLibs.native.crateNameFromCargoToml {src = workspacePath + "/projects/${builtins.elemAt includeProjects 0}";}) pname version;
    ...
    makeCratePackages = {
      ...
      native =
        ...
        |> makeNixPackage craneLibs.native
        |> (package: pkgs.callPackage package {});

      linuxX86_64 =
        crateArgs
        |> addArtifactsToArgs cargoArtifacts.linux.x86_64
        |> makeNixPackage craneLibs.linux.x86_64
      ...
      linuxAarch64 =
        ...
        |> makeNixPackage craneLibs.linux.aarch64
      ...
      macosX86_64 =
        ...
        |> makeNixPackage craneLibs.macos.x86_64
        |> (package: pkgs.callPackage package {})
      ...
      macosAarch64 =
        ...
        |> makeNixPackage craneLibs.macos.aarch64
        |> (package: pkgs.callPackage package {})
      ...
      windowsX86_64 =
        ...
        |> makeNixPackage craneLibs.windows.x86_64
        ...
        // {
          ...
          buildPhaseCargoCommand = ''
            ...
            HOME=$(mktemp -d fake-homeXXXX) ${pkgs.wasm-pack}/bin/wasm-pack build ./projects/${builtins.elemAt includeProjects 0} --out-dir $out/pkg
          '';
  ...
  in {
    packages =
      {
        rustDocumentation = craneLibs.native.cargoDoc (
          makeWorkspaceArgs "cargoDoc"
          {
    ...
    checks = {
      ...
      rustClippy = craneLibs.native.cargoClippy (
        makeWorkspaceArgs "cargoClippy"
        {
          inherit cargoArtifacts;
      ...
      rustFormat = craneLibs.native.cargoFmt (makeWorkspaceArgs "rustFormat" {});

      taploFmt = craneLibs.native.taploFmt (makeWorkspaceArgs "taploFmt" {
        src = pkgs.lib.sources.sourceFilesBySuffices workspacePath [".toml"];
      });

      cargoAudit = craneLibs.native.cargoAudit (makeWorkspaceArgs "cargoAudit" {
        advisory-db = config.forja.inputs.advisoryDb;
      });

      cargoDeny = craneLibs.native.cargoDeny (makeWorkspaceArgs "cargoDeny" {});

      cargoNextest = craneLibs.native.cargoNextest (
        makeWorkspaceArgs "cargoNextest"
        {
          inherit cargoArtifacts;
      ...
      cargoHakari = craneLibs.native.mkCargoDerivation (makeWorkspaceArgs "cargoHakari" {
        cargoArtifacts = null;
